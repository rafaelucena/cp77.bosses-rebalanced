...
local settings = {
    isBossesRebalancedEnabled = true,
...
registerForEvent("onInit", function()
    ...
    -- MAIN MOD SWITCH
    Override("RPGManager", "IsBossesRebalancedEnabled;", function ()
        return settings.isBossesRebalancedEnabled
    end)
...
function SetupMenu()
    ...
    if nativeSettings.pathExists("/RalphMods/bosses_rebalanced") then
        nativeSettings.removeSubcategory("/RalphMods/bosses_rebalanced")
    end
    nativeSettings.addSubcategory("/RalphMods/bosses_rebalanced", "Bosses Rebalanced")

    -- MAIN MOD SWITCH (path, label, desc, min, max, step, format, currentValue, defaultValue, callback, optionalIndex)
    nativeSettings.addSwitch(
        "/RalphMods/bosses_rebalanced",
        "Enable/Disable this mod",
        "Returning the bosses to it's original gameplay or change them to this mod configurations",
        settings.isBossesRebalancedEnabled,
        true,
        function(state)
            settings.isBossesRebalancedEnabled = state
        end,
        0
    )
